function(input, output) {
    ...
    data <- reactive({
        ...
        validate(
          need(input$fichero != "" && input$samples != "", "Please, select a file sample.")
        )
        ficheroCSV <- input$fichero;
        

        #Leemos el fichero
        x <- read.csv(ficheroCSV$datapath, header = TRUE);
        validate(
          need(length(x$volt) >= (input$samples), "El número de muestras seleccionadas es mayor que el número de muestras del fichero.")
        )
    

        #Array con las amplitudes (voltios)
        x1 <- x$volt[1:as.numeric(input$samples)];
        #Num de muestras
        N = length(x1);
        #Frecuencia de muestreo
        fs = N / 10;
        #Array con los tiempos
        t = x$time[1:as.numeric(input$samples)];
        ...
        if (N >= 2000) {
            ...
            c = 0;
            for (i in 1:length(left)) {
                if (i != 1 && i != length(left)) {
                    beats = timeValues[i + 1, 1] - timeValues[i, 1];
                    totalBeatsPerMinute = totalBeatsPerMinute + ((1 / beats) * 60);
                    c = c + 1;
                }
            }     
            totalBeatsPerMinute = totalBeatsPerMinute / c;
        }



        ###############################################################
        ## CÁLCULO DE ONDAS P Y T
        ###############################################################
                    ...
                    if (i < length(left) + 1) {
                        ...
                        if (distanceToR > (((totalBeatsPerMinute * 0.055) / 60) * 0.5) && distanceToR < (((totalBeatsPerMinute * 0.47) / 60) * 0.5)) {
                            ...
                            if (i == 1) {
                                ...
                                distanceBetweenActualRPeaks = RTimeValue2 - RTimeValue1;
                                distanceBetweenNextRPeaks = timeValues[i + 1, 1] - RTimeValue2;
                                distanceBetweenTwoNextRPeaks = timeValues[i + 2, 1] - timeValues[i + 1, 1];
                                distanceBetweenNextsRPeaks = mean(c(distanceBetweenNextRPeaks, distanceBetweenTwoNextRPeaks));

                                if (distanceBetweenActualRPeaks > (distanceBetweenNextsRPeaks - 0.1) && distanceBetweenActualRPeaks < (distanceBetweenNextsRPeaks + 0.1)) {
                        ...
                        if (distanceToR > RTmin && distanceToR < RTmax) {
                            ...
                            #Calculamos la distancia
                            distanceBetweenActualRPeaks = RTimeValue2 - RTimeValue1;
                            distanceBetweenPreviousRPeaks = RTimeValue1 - timeValues[i - 2, 1];
                            distanceBetweenTwoPreviousRPeaks = timeValues[i - 2, 1] - timeValues[i - 3, 1];
                            distanceBetweenPreviousRPeaks = mean(c(distanceBetweenPreviousRPeaks, distanceBetweenTwoPreviousRPeaks));

                            if (distanceBetweenActualRPeaks > (distanceBetweenPreviousRPeaks - 0.1) && distanceBetweenActualRPeaks < (distanceBetweenPreviousRPeaks + 0.1)) {
                                timeValueMax = t[indexOfMax[length(indexOfMax)]];
                                distanceToR = RTimeValue2 - timeValueMax;
                                if ((distanceToR > (((totalBeatsPerMinute * 0.055) / 60) * 0.5)) && (distanceToR < (((totalBeatsPerMinute * 0.47) / 60) * 0.5))) {
                                    #Almacenamos los valores P
                     ...
                     #Solo analizo si no estoy en la último posición. Si estoy en la última posición es que ya no hay más complejos QRS, por lo que no puedo detectar la onda P con seguridad
                    if (i < length(left) + 1) {
                        timeValueMax = t[indexOfMax[length(indexOfMax)]];
                        distanceToR = RTimeValue2 - timeValueMax;
        ...
        #Datos a retornar
        data <- list(x1 = x1, t = t, qrsColors = c("green", "blue", "red"), left = left, qrsTimeValues = timeValues, qrsValues = qrsValues,
            pValues = pValues,
            pTimeValues = pTimeValues,
            tValues = tValues,
            tTimeValues = tTimeValues,
            ptColors = c("purple", "orange"),
            totalBeatsPerMinute = totalBeatsPerMinute
        );
    ...
    output$plot1 <- renderPlot({
        ...
        plot(data$t, data$x1, type = "l", xlab = "time", ylab = "volts", ylim = c(-1, 1.4))
        legend(0, 1.4, c("P", "Q", "R", "S", "T"), pch = c(15, 15, 15, 15, 15), lwd = c(2.5, 2.5, 2.5, 2.5, 2.5), col = c("orange", "blue", "green", "red", "purple"))
        #legend(0, 1.2, c("P", "T"), lty = c(1, 1), lwd = c(2.5, 2.5), col = c("orange", "purple"))
        for (i in 1:length(data$left)) {
            for (x in 1:3) {
                points(data$qrsTimeValues[i, x], data$qrsValues[i, x], pch = 15, col = data$qrsColors[x]);
            }
        }
        for (i in 1:(length(data$left)+1)) {
            for (x in 1:3) {
                if (data$tTimeValues[i, x] != 0 && data$tValues[i, x] != 0)
                    points(data$tTimeValues[i, x], data$tValues[i, x], pch = 15, col = data$ptColors[1]);
                if (data$pTimeValues[i, x] != 0 && data$pValues[i, x] != 0)
                    points(data$pTimeValues[i, x], data$pValues[i, x], pch = 15, col = data$ptColors[2]);

            }
        }
        if (data$totalBeatsPerMinute > 0) {
            #text(x = 0.5, y = 1.4, labels = paste("Ppm:", round(data$totalBeatsPerMinute, digits = 2), sep = " "));
        }
